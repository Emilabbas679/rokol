			...
			<div class="sect_header clearfix">
				...
				<div class="order_links">
					<a href="{!! route('orders.index') !!}"
						class="{!! !request()->filled('status') ? 'active' : '' !!}">{{translate('all')}}</a>
					<a href="{!! route('orders.index', ['status' => \App\Models\ProductOrder::DELIVERED_STATUS_COMPLETED]) !!}"
						class="{!! activeClassByQueryParam('status', \App\Models\ProductOrder::DELIVERED_STATUS_COMPLETED)  !!}">{{translate('completed')}}</a>
					<a href="{!! route('orders.index', ['status' => \App\Models\ProductOrder::DELIVERED_STATUS_PREPARING]) !!}"
						class="{!! activeClassByQueryParam('status', \App\Models\ProductOrder::DELIVERED_STATUS_PREPARING)  !!}">{{translate('prepared')}}</a>
					<a href="{!! route('orders.index', ['status' => \App\Models\ProductOrder::DELIVERED_STATUS_CANCELED]) !!}"
						class="{!! activeClassByQueryParam('status', \App\Models\ProductOrder::DELIVERED_STATUS_CANCELED)  !!}">{{translate('cancelled')}}</a>
				</div>
			</div>
									...
									<div class="order_header">
										<div class="row">
											...
											@elseif($order->delivered_status == \App\Models\ProductOrder::DELIVERED_STATUS_PREPARING)
																	@php
																		$txt = translate("prepared");
																		$class = 'preparing'
																	@endphp
											@else
																	@php
																		$txt = translate("cancelled");
																		$class = 'rejected'
																	@endphp
											@endif
											<div class="col">
											...
											<div class="item_content">
												<div class="list_left_itm">
													<h4 class="itm_title">
														...
														<span
															class="itm_weight">{!! ($weight = $item->price?->weight)?->weight . " " . productWeightUnit($weight?->weight_type) !!}</span>
													</h4>
